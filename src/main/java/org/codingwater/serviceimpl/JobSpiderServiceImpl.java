...
public class JobSpiderServiceImpl implements IJobSpiderService {
	...
	private Logger logger = LoggerFactory.getLogger(JobSpiderServiceImpl.class);

	@Override
	public List<LagouJobInfo> fetchJobInfosFromLagou(String city, String keyword, int pageNumber, String monthlySalary,
			String workYears) {
		...
		String queryUrl = String.format(
				"http://www.lagou.com/jobs/positionAjax.json" + "?city=%s&first=true&px=new&pn=%d&kd=%s&gj=%s&yx=%s",
	...
	public List<NeituiJobInfo> fetchJobInfosFromNeitui(String city, String keyword, int pageNumber,
		...
		String neiTuiUrl = String.format(
				"http://www.neitui.me/"
	...
	private List<NeituiJobInfo> getJobListFromElements(Elements joblis) {
		List<NeituiJobInfo> ret = new ArrayList<>();
	...
	public List<LagouJobInfo> getJobListFromJson(String resultData) {
		List<LagouJobInfo> ret = new ArrayList<>();
		...
		for (Map<String, Object> job : jobIndoList) {
			...
			lagouJobInfo
					.setDetailPage(String.format("http://www.lagou.com/jobs/%s.html", lagouJobInfo.getPositionId()));
	...
	public void fetchYesterdayDataFromLagou(String keyword) {
		...
		List<LagouJobInfo> yesterdayJobList = new ArrayList<>();
		while (isContinue) {
			String queryUrl = String.format(
					"http://www.lagou.com/jobs/positionAjax.json" + "?first=true&px=new&kd=%s&pn=%d", keyword,
					pageNumber);
			...
			// 过滤
			List<LagouJobInfo> filtedList = lagouJobInfoList.stream().filter(predicate).collect(Collectors.toList());

			filtedList.forEach(job -> System.out.println(job.getCompanyName() + ":" + job.getCity() + ":"
	...
	public void saveToDataBase(List<LagouJobInfo> yesterdayJobList) {
		...
		for (LagouJobInfo job : yesterdayJobList) {
			List<BaseJobInfo> jobs = baseJobInfoDAO.execute(sb.toString(), job.getPositionId());
			if(jobs == null || jobs.isEmpty()) {
				baseJobInfoDAO.insert(job);
			}
		}
	}
	...
	public void fetchAndSaveDataFromNeitui(String keyword) {
		...
		while (isContinue) {
			...
			String neituiQueryUrl = String.format(
					"http://www.neitui.me/index" + ".php?name=neitui&handle=lists&page=%d" + "&kcity=全国&keyword=%s",
					pageNumber, keyword);
